    ...
    <div class="comboList">
        <div class="crumbs">
            <el-breadcrumb separator="/">
                ...
                <el-breadcrumb-item>组合商品列表</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <ShopNav :productList="productList" :type="1" :userData="this.userData" @updatePage="updatePage" :inputVal="inputValue">
            <!---->
            <template slot="add-btn">
                <el-button class="full-btn" type="primary" icon="el-icon-plus" @click.stop="createdProduct">新增组合商品</el-button>
            </template>
        </ShopNav>
        <div class="table-list">
            <el-table border :header-cell-style="styleObj" :cell-style="styleObj" ref="multipleTable" :data="comboList">
                ...
                <el-table-column prop="product_name" label="商品名称" width="320"></el-table-column>
        ...
        <!-- 操作记录 -->
        <el-dialog title="组合商品操作记录" :visible.sync="showRecords">
...
export default {
    ...
    mounted() {
        this.currentPage = this.$store.state.shopAdmin.comboListNum
        this.getComboList(this.$store.state.shopAdmin.comboListNum)
    },
    //进入edit
    beforeRouteLeave(to, from, next) {
        if (!to.path.endsWith('comboEdit')) {
            this.$store.dispatch('CHANGEINPUTVALUE', {
                inputValue: ""
            })
        }
        next()
    ...
    methods: {
        ...
        createdProduct() {
            ...
            this.$router.push({ path: 'comboEdit' })
        },
        toEdit(index, scope) {
            this.$store.commit('COMBOLISTNUM', {num: this.currentPage})
            this.$router.push({ path: 'comboEdit', query: { productId: scope.id_product, p_kkid: scope.p_kkid } })
        },
        getComboList(page_num = 1, page_size = 20) {
            let data = {
                is_combination: 1,
                key_word_type: this.selectType,                  // 关键字类型: 1商品id, 2商品名称.
                key_word: this.$store.state.shopAdmin.inputValue || '', // 关键字. 为空表示搜索所有
