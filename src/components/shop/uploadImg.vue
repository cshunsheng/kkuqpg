    ...
    <div id="upload-pic__container">
        ...
        <div class="imgList">
            <div class="imgBox" v-for="(item,index) in currentPicList" :key="index" v-dragging="{item:item,list:currentPicList,group:type,index:index}">
                ...
                <div class="imgFuns">
                    <i @click.stop="viewImg(item.id_product_kkh_url,index)" class="el-icon-zoom-in"></i>
                    <i @click.stop="deleteImg(item.id_product_kkh_url,index)" class="el-icon-delete"></i>
                </div>
            </div>
            <div class="imgbox imgInput" v-if="type === 'cover_pic' && currentPicList.length < 1">
                <img src="../../../static/img/ordrefuse.jpg">
                <input type="file" accept="image/jpg,image/jpeg,image/png" @change="selectImg"/>
            </div>
            <div class="imgbox imgInput" v-if="type !== 'cover_pic'">
                <img src="../../../static/img/ordrefuse.jpg">
                <input type="file" accept="image/jpg,image/jpeg,image/png" @change="selectImg"/>
            </div>
...
export default {
    ...
    methods: {
        // 上传图片
        selectImg(event){
            if(this.type === 'cover_pic' && this.currentPicList.length >=1) {
                this.$message.warning('展示图只限一张')
                return ;
            }
			...
			this.http('POST',formData,function(res){
				if(res.data){
                    let _imgUrl = [{id_product_kkh_url:res.data.image_url}];
